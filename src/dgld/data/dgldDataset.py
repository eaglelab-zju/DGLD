...
class NodeLevelAnomalyDataset(dgl.data.DGLDataset):
    ...
    def __init__(self, name, category: Literal['injected', 'natural', 'downsampled'], raw_dir=None, random_seed=42, verbose=True, transform=None, **kwargs):
        ...
        if category == 'injected':
            ...
            k =   kwargs.get('k') if (kwargs.get('k') is not None) else K 
            p =   kwargs.get('p') if (kwargs.get('p') is not None) else P 
            q =   kwargs.get('q') if (kwargs.get('q') is not None) else Q_MAP[name]
            
            graph = inject_contextual_anomalies(graph=graph,k=k,p=p,q=q)
            graph = inject_structural_anomalies(graph=graph,p=p,q=q)
            graph = [graph]
