...
def loss_func(x, x_hat, c, c_hat, h_a, h_s, hom_str, hom_attr, alphas, scale_factor, pretrain=False):
    ...
    # Eq.7
    loss = alphas[0] * loss_prox_str + alphas[1] * loss_hom_str + alphas[2] * loss_prox_attr + alphas[
        3] * loss_hom_attr + alphas[4] * loss_com

    score = (oa + os + oc) / 3
    return loss, score


def train_step(model, optimizer: torch.optim.Optimizer, g: dgl.DGLGraph, adj: torch.Tensor, batch_size: int,
               alphas: list, num_neighbors: int, device, pretrain=False):
    """
    ...
    for sg in dataloader:
        ...
        loss, score = loss_func(sub_adj, x_hat, feat, c_hat, h_a, h_s, hom_str, hom_attr, alphas, scale_factor,
                                pretrain=pretrain)
